...
void turnLogic2();
void cleanTurnError();
void goForward(float myDistance);
void goTurn(float myBearing);
...
float RtempDistance;
float RtotalDistance;//misnomer, we reset this after each movement
...
float PConstAngular=.6;//can be adjusted
float DConstAngular=.1;
// float PConstAngular=4;//can be adjusted
// float DConstAngular=5;
//final project variables
float criticalDistance=0.4;//if less than 0.3m, turn left, is more, turn right
float rangerDistance;//always 16 bits in centimeters
float firstHeading;
float desiredBearing;
float realFirstHeading;
...
// Place initialization code (or run-once) code here
  firstHeading=(readCompassHeading()/10.0);//car always starts where the compass is pointing
  realFirstHeading=firstHeading;
  printf("initial Heading: %5.2f | heading+90: %5.2f | heading-90: %5.2f\n",firstHeading,(firstHeading+90),(firstHeading-90));
...
//end of test condtions
  ...
  while(1){
        printf("\n new round\n");
      //we enter here when we have to make a new decision on where to go
      //car heading math will go away as the compass is directly attached to the car. we now know the angle of the car always
      delay_cycles(10e6); // Wait 1/10 of a second
      turnLogic2();
      forward2();
      //turnTest();
  }
...
void turnLogic2(){
    PConstAngular=0.6;
    if(readRanger()>35){//magic number
        forward2();//open space to move, move into it
    }else{
        firstHeading+=90;//turn clockwise (right
        printf("turning right by 90\n");
        goTurn(firstHeading);
        if(readRanger()<30){//still nothing, face the opposite direction
            firstHeading-=180;
            printf("turning left by 180\n");
            goTurn(firstHeading);
        }
    }
}
void forward2(){
    PConstAngular=0.1;
    desiredBearing=firstHeading;
    printf("forward2 \n");
      delay_cycles(3e6);//pause before action
    while(readRanger()>15){
        linearRPM=60;
        LdesiredRPM=60;
        RdesiredRPM=60;
    ...
    printf("stopped forward2 \n");

}
void goTurn(float myBearing){
  printf("attempting to turn\n");
  delay_cycles(3e6);
  linearRPM=0;

  if(myBearing>360){//prevents myBearing from being an impossible value
      myBearing-=360;
  }else if(myBearing<0){
      myBearing+=360;
  }
  carHeading=readCompassHeading()/10.0;//compass reads from 0 to 359.9.
//   printf("current Bearing %5.2f | ",carHeading);
//   printf("desired heading: %5.2f\n",myBearing);//arguement for the function
  desiredBearing=myBearing;//now I can use this variable in another function
  headingError=desiredBearing-carHeading;
  cleanTurnError();
  printf("initial heading error: %5.2f\n\n",(headingError));
    printf("current Bearing %5.2f | ",carHeading);
    printf("desired heading: %5.2f\n",myBearing);//arguement for the function
  while(abs(headingError)>2){
      //printf("inside turn while loop\n");
      turnControl2();
      leftPID();
      rightPID();
      delay_cycles(3.2e6);
...
void turnControl2(){//instead of car heading and compass heading, its car heading and desired bearing.
  carHeading=readCompassHeading()/10.0;//compass reads from 0 to 359.9.
   headingError=desiredBearing-carHeading;//desired-actual
...
//   printf("heading error: %5.2f | ",headingError);
//   printf("derivative term:% 5.2f \n",(headingError-previousHeadingError));

  if(turnRatio>60){
   turnRatio=60;
  }else if(turnRatio<-60){
   turnRatio=-60;
  }

  //printf("turnRatio: %5.2f\n",turnRatio);
   //turnInfo();
...
uint16_t readCompassHeading(){
  volatile uint8_t results1=I2C_readData(I2C1,0x60,2,dataArray,2);
   uint16_t temp2=(dataArray[0]<<8)+dataArray[1];
   if(temp2/10.0>360){
       return previousCompassHeading;
   }
   previousCompassHeading=(temp2/10.0);
  return temp2;//bitshift the dataArray to make room for the other data
  //void I2C_readData( I2C_Regs * i2c , uint8_t PeriphAddress , uint8_t StartReg , uint8_t * data , uint8_t len )
}

uint16_t readRanger(){
  dataArray[0]=0x51;
  volatile uint8_t stupid=I2C_writeData(I2C1,0x70,0,dataArray,1);
  delay_cycles(3.2e6); // Wait 1/10 of a second

  volatile uint8_t results2=I2C_readData(I2C1,0x70,2,dataArray,2);
  uint16_t temp1=(dataArray[0]<<8)+dataArray[1];
...
void turnInfo(){
  printf("Car Heading: %5.2f | ",carHeading);
  printf("Desired Bearing: %5.2f | ",desiredBearing);
